                            ...
                            <table class="table table-striped table-bordered" id="dataTable" width="100%"
                                ...
                                <tbody>
                                    ...
                                    SELECT 
                                        ...
                                        m.nama_mahasiswa,
                                        CASE
                                            WHEN pj.status_pengumpulan_publikasi_jurnal = 'ditolak' OR a.status_pengumpulan_aplikasi = 'ditolak' OR s.status_pengumpulan_skripsi = 'ditolak'
                                                THEN 'ditolak'
                                            WHEN pj.status_pengumpulan_publikasi_jurnal = 'pending' OR a.status_pengumpulan_aplikasi = 'pending' OR s.status_pengumpulan_skripsi = 'pending'
                                                THEN 'pending'
                                            WHEN pj.status_pengumpulan_publikasi_jurnal = 'belum upload' OR a.status_pengumpulan_aplikasi = 'belum upload' OR s.status_pengumpulan_skripsi = 'belum upload'
                                                THEN 'belum upload'
                                            ELSE 'terverifikasi'
                                        END AS status,
                                        CONCAT(
                                    ...
                                    ORDER BY 
                                        CASE 
                                            WHEN COALESCE(pj.status_pengumpulan_publikasi_jurnal, 'belum upload') = 'pending' THEN 1
                                            WHEN COALESCE(pj.status_pengumpulan_publikasi_jurnal, 'belum upload') = 'ditolak' THEN 2
                                            WHEN COALESCE(pj.status_pengumpulan_publikasi_jurnal, 'belum upload') = 'belum upload' THEN 3
                                            WHEN COALESCE(pj.status_pengumpulan_publikasi_jurnal, 'belum upload') = 'terverifikasi' THEN 4
                                            ELSE 5
                                        END,
                                        CASE 
                                            WHEN COALESCE(a.status_pengumpulan_aplikasi, 'belum upload') = 'pending' THEN 1
                                            WHEN COALESCE(a.status_pengumpulan_aplikasi, 'belum upload') = 'ditolak' THEN 2
                                            WHEN COALESCE(a.status_pengumpulan_aplikasi, 'belum upload') = 'belum upload' THEN 3
                                            WHEN COALESCE(a.status_pengumpulan_aplikasi, 'belum upload') = 'terverifikasi' THEN 4
                                            ELSE 5
                                        END,
                                        CASE 
                                            WHEN COALESCE(s.status_pengumpulan_skripsi, 'belum upload') = 'pending' THEN 1
                                            WHEN COALESCE(s.status_pengumpulan_skripsi, 'belum upload') = 'ditolak' THEN 2
                                            WHEN COALESCE(s.status_pengumpulan_skripsi, 'belum upload') = 'belum upload' THEN 3
                                            WHEN COALESCE(s.status_pengumpulan_skripsi, 'belum upload') = 'terverifikasi' THEN 4
                                            ELSE 5
                                        END;
                                ";                                
                                    ...
                                    while ($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)) {
                                        ...
                                        echo "<td><a href='detail_mahasiswa.php?nim=" . urlencode($nim) . "' class='btn btn-primary btn-sm' target='_blank'>Detail</a></td>";
                                        echo "</tr>";
                                    }
    ...
    <script>
        ...
        $(document).ready(function () {
            var table = $('#dataTable').DataTable({
                ...
                "order": [[3, 'asc']], // Urutkan berdasarkan kolom status (indeks 3)
                "columnDefs": [
                    {
                        "targets": 3,
                        "type": "num",  // Menetapkan tipe pengurutan numerik pada kolom status
                        "orderData": [3]
                    }
                ]
            });
            ...
            $('#statusFilter').on('change', function () {
                var status = $(this).val();
                table.column(3).search(status).draw(); // Kolom ke-3 adalah Status
            });
        });
